...
export default Service.extend({
  ...
  addMessage(options, resolve, reject) {
    ...
    const message = EmberObject.create({
      ...
      beforeClose: isEmpty(options.beforeClose)
        ? undefined
        : options.beforeClose,
      distinguishCancelAndClose: isEmpty(options.distinguishCancelAndClose)
        ? null
        : options.distinguishCancelAndClose,
      showCancelButton: isEmpty(options.showCancelButton)
        ? false
        : options.showCancelButton,
      showConfirmButton: isEmpty(options.showConfirmButton)
        ? true
        : options.showConfirmButton,
      cancelButtonText: isEmpty(options.cancelButtonText)
        ? 'Cancel'
        : options.cancelButtonText,
      confirmButtonText: isEmpty(options.confirmButtonText)
        ? 'Ok'
        : options.confirmButtonText,
      cancelButtonClass: isEmpty(options.cancelButtonClass)
        ? null
        : options.cancelButtonClass,
      confirmButtonClass: isEmpty(options.confirmButtonClass)
        ? null
        : options.confirmButtonClass,
      closeOnClickModal: isEmpty(options.closeOnClickModal)
        ? false
        : options.closeOnClickModal,
      closeOnPressEscape: isEmpty(options.closeOnPressEscape)
        ? false
        : options.closeOnPressEscape,
      showInput: isEmpty(options.showInput) ? null : options.showInput,
      inputPlaceholder: isEmpty(options.inputPlaceholder)
        ? null
      ...
      inputValidator: isEmpty(options.inputValidator)
        ? null
        : options.inputValidator,
      inputErrorMessage: isEmpty(options.inputErrorMessage)
        ? null
