...
export default Component.extend({
  ...
  attributeBindings: ['role', 'ariaModal:aria-modal', 'ariaLabel', 'tabindex'],
  role: 'dialogue',
  ariaModal: true,
  tabindex: -1,
  ...
  icon: computed('messageObj.iconClass.type', 'messageObj.type', function () {
    let typeMap = {
      success: 'success',
      info: 'info',
      warning: 'warning',
  ...
  getClassNameForBox: computed(
    'messageObj.{customClass,center,dismiss}',
    function () {
      let classNames = 'animated ';
      if (get(this, 'messageObj.customClass')) {
        classNames += ' ' + get(this, 'messageObj.customClass');
      }
      if (get(this, 'messageObj.center')) {
  ...
  doAfterClose() {
    let currentMsg = this.messageObj;
    if (currentMsg.resolve) {
      let action = this.action;
      if (action === 'confirm') {
        if (currentMsg.showInput) {
          currentMsg.resolve({ value: currentMsg.inputValue, action });
        } else {
  ...
  handleAction(action) {
    if (
      get(this, 'messageObj.type') === 'prompt' &&
      action === 'confirm' &&
      !this.validate()
    ) {
      return;
    ...
    if (typeof get(this, 'messageObj.beforeClose') === 'function') {
      ...
      get(this, 'messageObj.beforeClose')(action, this.messageObj, this);
    } else {
      this.doClose();
    }
  },
  ...
  validate() {
    if (get(this, 'messageObj.type') === 'prompt') {
      ...
      if (
        inputPattern &&
        !inputPattern.test(get(this, 'messageObj.inputValue') || '')
      ) {
        set(
          this,
          'messageObj.editorErrorMessage',
          get(this, 'messageObj.inputErrorMessage')
        );
      ...
      if (typeof inputValidator === 'function') {
        const validateResult = inputValidator(
          get(this, 'messageObj.inputValue')
        );
  ...
  actions: {
    handleWrapperClick() {
      if (this.closeOnClickModal) {
        this.handleAction(
          get(this, 'messageObj.distinguishCancelAndClose') ? 'close' : 'cancel'
        );
      }
    },
  ...
  click(e) {
    if (
      ...
      get(this, 'messageObj.closeOnClickModal')
    ) {
      this.handleAction(
        get(this, 'messageObj.distinguishCancelAndClose') ? 'close' : 'cancel'
      );
